...
<div class="container-fluid">
    <div>
        ...
        <c:if test = "${records > 0}">
            <div style="font-weight: bold; color: #FFFFFF; margin-left: 900px; border: none">New Provider Found:<c:out value = "${newProvi}"/></div>
        </c:if>
    </div>
<table id="dtBasicExample" class="table table-striped table-bordered table-sm table-background" cellspacing="0">
    ...
    <c:forEach items="${feedlist}" var="fdlist">
        <tr id="tableRow">
            <td>${fdlist.title}</td>
            <td><a href="${fdlist.link}" target="_blank">${fdlist.link}</a></td>
            <td>${fdlist.date}</td>
